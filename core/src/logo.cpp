...
namespace DreamEngine {
    OpeningPresentation::OpeningPresentation(const Camera *camera, const GeometryProxy &geometry) : camera(camera) {
        // Get the renderer
    ...
    void OpeningPresentation::onUpdate(float interval) {
        if (openingLogo->hasFinished) {
            sceneWillTransition = true;
            SceneManager::getInstance()->switchCurrentScene(nullptr);
        }
    }

    void OpeningPresentation::onExit() { SKIPPED }
}